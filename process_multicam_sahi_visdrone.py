"""
Process all 5 cameras using SAHI + VisDrone-optimized YOLOv8

VisDrone model is specifically trained on aerial/drone footage:
- Better detection of small vehicles from elevated angles
- 10 vehicle classes: car, van, truck, bus, motor, bicycle, etc.
- Expected 10-30% improvement over base YOLOv8
"""

import cv2
from sahi import AutoDetectionModel
from sahi.predict import get_sliced_prediction
from pathlib import Path
import pandas as pd
from tqdm import tqdm
import os

def download_visdrone_model():
    """Download VisDrone model if not already present"""
    try:
        from ultralyticsplus import YOLO
        print("📦 Loading VisDrone model from HuggingFace...")
        print("   This may take a few minutes on first run...")
        
        # Load the model - this will auto-download
        model = YOLO('mshamrai/yolov8s-visdrone')
        
        # Save locally for SAHI to use
        model_path = 'yolov8s-visdrone.pt'
        
        print(f"✅ VisDrone model ready!")
        print(f"   10 classes: pedestrian, people, bicycle, car, van, truck,")
        print(f"              tricycle, awning-tricycle, bus, motor")
        
        return 'mshamrai/yolov8s-visdrone'
        
    except Exception as e:
        print(f"⚠️  Error loading VisDrone model: {e}")
        print(f"   Falling back to standard YOLOv8...")
        return 'yolov8m.pt'

def process_video_with_sahi(video_path,
                            model_path='mshamrai/yolov8s-visdrone',
                            slice_height=320,
                            slice_width=320,
                            confidence=0.25,
                            save_every=50,
                            output_dir='output'):
    """
    Process video using SAHI sliced detection with VisDrone model
    
    Args:
        video_path: Path to video file
        model_path: Model to use (default: VisDrone-optimized)
        slice_height/width: Size of slices (smaller = more detections, slower)
        confidence: Confidence threshold
        save_every: Process every Nth frame
        output_dir: Where to save results
    """
    
    video_name = Path(video_path).stem
    print(f"\n{'='*70}")
    print(f"🎬 Processing: {video_name}")
    print(f"{'='*70}")
    
    # Load detection model
    print(f"\n📦 Loading VisDrone model...")
    
    try:
        # Try loading as VisDrone model
        if 'visdrone' in model_path.lower():
            from ultralyticsplus import YOLO
            yolo_model = YOLO(model_path)
            
            # Configure for max detections
            yolo_model.overrides['conf'] = confidence
            yolo_model.overrides['iou'] = 0.45
            yolo_model.overrides['agnostic_nms'] = False
            yolo_model.overrides['max_det'] = 1000  # VisDrone supports up to 1000!
            
            # Save temporarily for SAHI
            temp_model_path = 'temp_visdrone.pt'
            yolo_model.model.save(temp_model_path)
            
            detection_model = AutoDetectionModel.from_pretrained(
                model_type='yolov8',
                model_path=temp_model_path,
                confidence_threshold=confidence,
                device='cuda:0'
            )
        else:
            # Fallback to standard model
            detection_model = AutoDetectionModel.from_pretrained(
                model_type='yolov8',
                model_path=model_path,
                confidence_threshold=confidence,
                device='cuda:0'
            )
    except Exception as e:
        print(f"⚠️  Could not load VisDrone model: {e}")
        print(f"   Using standard YOLOv8 instead...")
        detection_model = AutoDetectionModel.from_pretrained(
            model_type='yolov8',
            model_path='yolov8m.pt',
            confidence_threshold=confidence,
            device='cuda:0'
        )
    
    # Open video
    cap = cv2.VideoCapture(str(video_path))
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    fps = int(cap.get(cv2.CAP_PROP_FPS))
    
    print(f"📹 Video info:")
    print(f"   Total frames: {total_frames}")
    print(f"   FPS: {fps}")
    print(f"   Processing every {save_every} frames")
    
    frames_to_process = list(range(0, total_frames, save_every))
    print(f"   Will process: {len(frames_to_process)} frames")
    
    # Process frames
    detections = []
    temp_file = 'temp_frame.jpg'
    
    print(f"\n🔍 Running SAHI detection (slice size: {slice_width}x{slice_height})...")
    print(f"   Using VisDrone-optimized model for aerial footage...")
    
    for frame_num in tqdm(frames_to_process, desc=f"   {video_name}"):
        cap.set(cv2.CAP_PROP_POS_FRAMES, frame_num)
        ret, frame = cap.read()
        
        if not ret:
            continue
        
        # Save frame temporarily
        cv2.imwrite(temp_file, frame)
        
        # Run SAHI sliced detection
        result = get_sliced_prediction(
            temp_file,
            detection_model,
            slice_height=slice_height,
            slice_width=slice_width,
            overlap_height_ratio=0.2,
            overlap_width_ratio=0.2,
            verbose=0
        )
        
        # Extract detections
        for pred in result.object_prediction_list:
            detections.append({
                'frame': frame_num,
                'time_sec': frame_num / fps,
                'class': pred.category.name,
                'confidence': pred.score.value,
                'x1': int(pred.bbox.minx),
                'y1': int(pred.bbox.miny),
                'x2': int(pred.bbox.maxx),
                'y2': int(pred.bbox.maxy),
                'width': int(pred.bbox.maxx - pred.bbox.minx),
                'height': int(pred.bbox.maxy - pred.bbox.miny)
            })
    
    cap.release()
    
    # Cleanup temp files
    if os.path.exists(temp_file):
        os.remove(temp_file)
    if os.path.exists('temp_visdrone.pt'):
        os.remove('temp_visdrone.pt')
    
    # Save results
    df = pd.DataFrame(detections)
    
    if len(df) > 0:
        output_path = Path(output_dir) / f'sahi_visdrone_{video_name}.csv'
        output_path.parent.mkdir(parents=True, exist_ok=True)
        df.to_csv(output_path, index=False)
        
        # Statistics
        print(f"\n📊 Results:")
        print(f"   Total detections: {len(df)}")
        print(f"   Avg per frame: {len(df) / len(frames_to_process):.1f}")
        print(f"   Classes: {df['class'].value_counts().to_dict()}")
        print(f"   Saved to: {output_path}")
    else:
        print(f"\n⚠️  No detections found")
    
    return df

def process_all_cameras(video_dir='C:/Users/sakth/Documents/traffique_footage',
                       slice_size=320,
                       save_every=50):
    """
    Process all 5 camera videos with SAHI + VisDrone
    """
    print("="*70)
    print("🎥 MULTI-CAMERA SAHI + VISDRONE PROCESSING")
    print("="*70)
    
    print(f"\n⚙️  Configuration:")
    print(f"   Model: VisDrone YOLOv8s (aerial-optimized)")
    print(f"   Slice size: {slice_size}x{slice_size}")
    print(f"   Save every: {save_every} frames")
    print(f"   Confidence: 0.25")
    print(f"   Max detections: 1000 per frame")
    
    # Download/load VisDrone model
    model_path = download_visdrone_model()
    
    # Find videos
    video_path = Path(video_dir)
    videos = sorted(video_path.glob('D*F*_stab.mp4'))
    
    if not videos:
        print(f"\n❌ No videos found in {video_dir}")
        return
    
    print(f"\n📹 Found {len(videos)} videos:")
    for v in videos:
        print(f"   - {v.name}")
    
    # Process each video
    all_results = {}
    
    for video in videos:
        df = process_video_with_sahi(
            video,
            model_path=model_path,
            slice_height=slice_size,
            slice_width=slice_size,
            save_every=save_every
        )
        all_results[video.stem] = df
    
    # Overall summary
    print("\n" + "="*70)
    print("📊 OVERALL SUMMARY")
    print("="*70)
    
    total_detections = sum(len(df) for df in all_results.values())
    total_frames = sum(len(df['frame'].unique()) if len(df) > 0 else 0 
                      for df in all_results.values())
    
    print(f"\n🎯 Total across all cameras:")
    print(f"   Detections: {total_detections}")
    print(f"   Frames processed: {total_frames}")
    print(f"   Avg per frame: {total_detections / total_frames:.1f}" if total_frames > 0 else "   No frames")
    
    print(f"\n📹 Per camera:")
    for video_name, df in all_results.items():
        if len(df) > 0:
            frames = len(df['frame'].unique())
            print(f"   {video_name}: {len(df)} detections ({len(df)/frames:.1f} per frame)")
        else:
            print(f"   {video_name}: 0 detections")
    
    print("\n" + "="*70)
    print("✅ PROCESSING COMPLETE!")
    print("="*70)
    
    print(f"\n📁 Results saved in: output/sahi_visdrone_*.csv")
    print(f"\n💡 Compare with standard SAHI:")
    print(f"   Standard: output/sahi_*.csv")
    print(f"   VisDrone: output/sahi_visdrone_*.csv")

if __name__ == '__main__':
    print("="*70)
    print("🔪 SAHI + VISDRONE MULTI-CAMERA PROCESSOR")
    print("="*70)
    
    print("""
VisDrone Model Benefits:
- Trained specifically on aerial/drone footage
- Optimized for small vehicles from elevated angles
- 10 vehicle classes including pedestrians
- Max 1000 detections per frame
- Expected 10-30% improvement over base YOLOv8

Expected Results:
- Standard YOLOv8: 2-6 vehicles
- YOLOv8 + SAHI: 82 vehicles
- VisDrone + SAHI: 90-120+ vehicles! 🚀
""")
    
    print("⚙️  Choose slice size:")
    print("   1. 320x320 - Maximum detections, slower (~3-4x slower)")
    print("   2. 640x640 - Good balance, faster (~2x slower)")
    print("   3. Custom")
    
    choice = input("\nChoice (1/2/3) [default=2]: ").strip() or '2'
    
    if choice == '1':
        slice_size = 320
    elif choice == '3':
        slice_size = int(input("Enter slice size (e.g., 320, 480, 640): "))
    else:
        slice_size = 640
    
    print(f"\nHow often to process frames?")
    print(f"   1. Every frame (slowest, most complete)")
    print(f"   2. Every 25 frames (~1 second at 25fps)")
    print(f"   3. Every 50 frames (~2 seconds) - RECOMMENDED")
    print(f"   4. Every 100 frames (~4 seconds)")
    
    choice = input("\nChoice (1/2/3/4) [default=3]: ").strip() or '3'
    
    save_every_map = {'1': 1, '2': 25, '3': 50, '4': 100}
    save_every = save_every_map.get(choice, 50)
    
    print(f"\n🚀 Starting VisDrone + SAHI processing...")
    print(f"   Slice size: {slice_size}x{slice_size}")
    print(f"   Process every: {save_every} frames")
    
    # Estimate time
    print(f"\n⏱️  Estimated time per camera:")
    if slice_size == 320:
        print(f"   ~2-3 hours (135 slices per frame)")
    else:
        print(f"   ~1-1.5 hours (32 slices per frame)")
    print(f"   Total for 5 cameras: ~5-10 hours")
    print(f"   Can run overnight!")
    
    response = input("\nStart processing? (y/n): ").lower()
    
    if response == 'y':
        process_all_cameras(slice_size=slice_size, save_every=save_every)
    else:
        print("\n✅ Processing cancelled")
        print(f"   Run when ready: python process_multicam_sahi_visdrone.py")
